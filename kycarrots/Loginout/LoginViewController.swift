...
class LoginViewController: UIViewController {
    ...
    func unlinkKakaoAndServer(completion: (() -> Void)? = nil) {
        // 0) 먼저 providerUserId 확보 (로컬에 없으니 me()에서 가져오는게 안전)
        UserApi.shared.me { [weak self] user, meError in
            guard let self else { return }
            
            if let meError = meError {
                print("KAKAO me 실패:", meError)
                DispatchQueue.main.async {
                    self.presentKakaoAlert(title: "사용자 조회 실패", message: "카카오 사용자 정보를 가져오지 못했습니다.")
                    completion?()
                }
                return
            }
            
            let providerUserId = String(user?.id ?? 0)
            if providerUserId == "0" {
                DispatchQueue.main.async {
                    self.presentKakaoAlert(title: "사용자 조회 실패", message: "providerUserId가 없습니다.")
                    completion?()
                }
                return
            }
            ...
            UserApi.shared.unlink { [weak self] error in
                ...
                UserApi.shared.logout { logoutError in
                    ...
                    // 3) 우리 앱 로컬 저장 초기화 (기존 로직 그대로)
                    LoginInfoUtil.clearLoginInfo()
                    TokenUtil.clearToken()
                    
                    // 4) ✅ 서버 tb_social_account 삭제 호출 (추가된 핵심)
                    Task {
                        let req = UnlinkSocialRequest(provider: "KAKAO", providerUserId: providerUserId)
                        let res = await self.appService.unlinkSocial(req) // SimpleResultResponse?
                        ...
                        await MainActor.run {
                            if res?.result == true {
                                ...
                                self.presentKakaoAlert(
                                    title: "카카오 연결 해제 완료",
                                    message: "로컬 정보는 삭제했지만,\n서버 링크 삭제 실패: \(res?.message ?? "-")\n다시 시도해주세요."
                                )
                            }
                            completion?()
                        }
